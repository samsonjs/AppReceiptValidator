...
public struct InAppPurchaseReceipt: Equatable {
    ...
    /// Indicates whether intro pricing is in effect for this purchase. ASN.1 Field Type 1719.
    /// This key is only present for auto-renewable subscription receipts. The value for this key is "true"
    /// if the customer’s subscription is currently in an introductory price period, or "false" if not.
    ///
    /// - Note: If a previous subscription period in the receipt has the value “true” for either the
    ///         is_trial_period or the is_in_intro_offer_period key, the user is not eligible for a free
    ///         trial or introductory price within that subscription group.
